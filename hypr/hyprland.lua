-- Reload config with `hyprctl reload`
...
-- `hyprctl monitors` lists currently connected displays
--
-- `hyprdynamicmonitors tui` can generate and apply hyprland monitor profiles automatically
-- `nwg-displays` is a lightweight GUI to generate monitor profiles for Hyprland
--
-- `kanshi` can run alongside hyprland and override its internal monitor profile
hl.monitor({
    output   = "",
    mode     = "preferred",
    position = "auto",
...
local fileManager = "nautilus -w || cosmic-files || dolphin"
...
hl.config({
    general = {
        -- Gaps look cool in screenshots,
        -- but i feel like i'm wasting space on smaller displays
        -- and seeing tiny strips of wallpaper between windows
        -- is distracting (i should try changing wallpaper though).
        gaps_in  = 2,
        gaps_out = 4,
...
hl.config({
    misc = {
        ...
        disable_splash_rendering = true, -- Don't print fortune cookie text over wallpaper
    },
})
...
hl.bind(mainMod .. " + O", hl.dsp.exec_cmd("wlrctl window focus md.obsidian.Obsidian || obsidian || flatpak run md.obsidian.Obsidian"))
...
hl.bind(mainMod .. " + CTRL + Escape", hl.dsp.exec_cmd("noctalia msg panel-toggle session"))
-- Default Hyprland exit keybind
...
-- Screen capture
hl.bind("Print", hl.dsp.exec_cmd("flatpak run --env=WAYSHOT_CAPTURE=1 io.github.gutopardini.wayshot || flatpak run org.flameshot.Flameshot gui"))
